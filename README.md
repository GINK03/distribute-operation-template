...
メンツは、キャッシュさん、yu4uさん、私という激強のお二人に私が計算リソースの最適化で参加しました。画像のことはディープ以降の知識レベルであったので、大変勉強になったコンペです。結果は銀メダル２個です。　　
...
大量のクラウドの**一時的なインスタンスをかり**て、**SSHFSというファイルシステムで一つのマシンをマウント**し、**各インスタンスに処理の命令を送り**、**オーバーヘッドを見極め**て、**アルゴリズム的改善を行い**、改善した処理プロセスを行う、というPDCAのような流れをおこうなのですが、各要素について説明したいと思います。
...
for i in range(0, 3):
  ...
  os.system(ctx)
```
このスクリプトは、自分で作成した必要なライブラリがインストールされた状態のイメージ(nardtree-jupyter-1)からハイパフォーマンスのインスタンスを３台作成します。
...
## 5.5 コード
- [https://github.com/GINK03/distribute-operation-template:embed]
